...
public sealed class ZadaniaLinq
{
    ...
    {
        var res = DaneUczelni.Studenci.Where(s => s.Miasto.Equals("Warsaw")).Select(
            s => $"{s.NumerIndeksu}, {s.Imie}, {s.Nazwisko}, {s.Miasto}"
            );
        return res;
    ...
    {
        var res = DaneUczelni.Studenci.Select(s => s.Email);
        return res;
    ...
    {
        var res = DaneUczelni.Studenci.OrderBy(s => s.Nazwisko).ThenBy(s => s.Imie).Select(
            s => $"{s.NumerIndeksu}, {s.Imie}, {s.Nazwisko}"
            );
        return res;
    ...
    {
        var res = DaneUczelni.Przedmioty.Where(s => s.Kategoria.Equals("Analytics")).Take(1).Select(s=> s.Nazwa + "-" + s.DataStartu);
        if (res.Count(s => true) != 0)
        {
            return res;
        }
    ...
    {
        var res = DaneUczelni.Zapisy.Any(zapis => zapis.CzyAktywny.Equals(false)) ? "1" : "0";
        return [res];
    ...
    {
        var res = DaneUczelni.Prowadzacy.All(p => !string.IsNullOrEmpty(p.Katedra)) ? "1" : "0";
        return [res];
    ...
    {
        var res = DaneUczelni.Zapisy.Count(z => z.CzyAktywny).ToString();
        return [res];
    ...
    {
        var res = DaneUczelni.Studenci.Select(s => s.Miasto).Distinct().OrderBy(m=>m).ToList();
        return res;
    ...
    {
        var res = DaneUczelni.Zapisy.OrderByDescending(z => z.DataZapisu).Take(3).Select(z =>
            $"{z.DataZapisu:yyyy-MM-dd}, Student {z.StudentId}, Przedmiot {z.PrzedmiotId}");
        return res;
    ...
    {
        var res = DaneUczelni.Przedmioty.OrderBy(p => p.Nazwa).Skip(2).Take(2).Select(p => $"{p.Nazwa}, {p.Kategoria}").ToList();
        return res;
    ...
    {
        var res = DaneUczelni.Studenci.Join(DaneUczelni.Zapisy, s => s.Id, z => z.StudentId,
            (s, z) => $"{s.Imie}, {s.Nazwisko}, {z.DataZapisu:yyyy-MM-dd}").ToList();
        return res;
    ...
    {
        var res = DaneUczelni.Zapisy.Join(DaneUczelni.Przedmioty, z => z.PrzedmiotId, p => p.Id, (z, p) => p.Nazwa)
            .GroupBy(n => n).Select(g => $"{g.Key}, {g.Count()}").ToList();
        return res;
    ...
    {
        var res = DaneUczelni.Zapisy.Where(z =>  z.OcenaKoncowa != null)
            .Join(DaneUczelni.Przedmioty, z => z.PrzedmiotId, p => p.Id, (z, p) => new {p.Nazwa, z.OcenaKoncowa})
            .GroupBy(x => x.Nazwa)
            .Select(g => $"{g.Key}, {g.Average(z => z.OcenaKoncowa)}").ToList();
        return res;
    ...
    public IEnumerable<string> Zadanie15_ProwadzacyILiczbaPrzedmiotow()
    {
        var res = DaneUczelni.Prowadzacy
            .GroupJoin(DaneUczelni.Przedmioty, pr => pr.Id, p => p.ProwadzacyId, (pr, przedmioty) => new
            {
                pr.Imie,
                pr.Nazwisko,
    ...
    {
        var res = DaneUczelni.Studenci.Join(
                DaneUczelni.Zapisy.Where(z => z.OcenaKoncowa != null),
                s => s.Id, z => z.StudentId, (s, z) => new { s.Imie, s.Nazwisko, z.OcenaKoncowa }
            ).GroupBy(sz => new { sz.Imie, sz.Nazwisko })
            .Select(sz => $"{sz.Key.Imie}, {sz.Key.Nazwisko}, {sz.Max(sz => sz.OcenaKoncowa)}");
        return res;
    ...
    public IEnumerable<string> Wyzwanie01_StudenciZWiecejNizJednymAktywnymPrzedmiotem()
    {
        var res = DaneUczelni.Studenci
            .Join(DaneUczelni.Zapisy.Where(z => z.CzyAktywny), s => s.Id, z => z.StudentId, (s, z) => new {s, z})
            .GroupBy(z => new {z.s.Imie, z.s.Nazwisko})
            .Where(g => g.Count() > 1)
    ...
    public IEnumerable<string> Wyzwanie02_PrzedmiotyStartujaceWKwietniuBezOcenKoncowych()
    {
        var res = DaneUczelni.Przedmioty
            ...
            .Join(DaneUczelni.Zapisy, p => p.Id, z => z.PrzedmiotId, (p, z) => new { p.Nazwa, z })
            .GroupBy(x => new { x.Nazwa })
            .Where(z => z.All(z2 => z2.z.OcenaKoncowa == null))
            .Select(z => $"{z.Key.Nazwa}");
        return res;
    ...
    public IEnumerable<string> Wyzwanie03_ProwadzacyISredniaOcenNaIchPrzedmiotach()
    {
        var res = DaneUczelni.Prowadzacy
            ...
            .Select(x => $"{x.Imie} {x.Nazwisko}, {(x.Oceny.Any() ? x.Oceny.Average().ToString() : "brak ocen")}")
            .ToList();
        return res;
    ...
    {
        var res = DaneUczelni.Studenci.Join(DaneUczelni.Zapisy.Where(z => z.CzyAktywny), s => s.Id, z => z.StudentId,
                (s, z) => new { s, z })
            .GroupBy(x => new { x.s.Miasto})
            .OrderByDescending(z => z.Count())
            .Select(x => $"{x.Key.Miasto}, {x.Count()}");
        return res;
